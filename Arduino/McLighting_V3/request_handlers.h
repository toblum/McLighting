...
void onWsEvent(AsyncWebSocket * server, AsyncWebSocketClient * client, AwsEventType type, void * arg, uint8_t *payload, size_t len){
  if(type == WS_EVT_CONNECT){
    ...
    if(info->opcode == WS_TEXT) {
      ...
      if(payload[0] == '#') { // # ==> Set main color
        ...
        #ifdef ENABLE_AMQTT
          if(mqttClient.connected()) mqttClient.publish(mqtt_outtopic.c_str(), qospub, false, String(String("OK ") + payload_str).c_str());
        #endif
         #ifdef ENABLE_HOMEASSISTANT
      ...
      } else if (payload_str.startsWith("=off") ) { // off state
        ws2812fx_mode = FX_MODE_STATIC;
        stateOn = false;
        mode = OFF;
        client->text("OK");
      ...
      } else if ( payload_str.startsWith("start") ) { // start auto cycling
        handleAutoStart();
        client->text("OK");
        
      } else if ( payload_str.startsWith("stop") ) { // stop auto cycling
...
#ifdef ENABLE_AMQTT
  void connectToWifi() {
    DEBUG_PRINTLN("Re-connecting to Wi-Fi...");
  ...
  void onWifiDisconnect(const WiFiEventStationModeDisconnected& event) {
    ...
    #endif
    taskConnecttMqtt.disable();; // ensure we don't reconnect to MQTT while reconnecting to Wi-Fi
    taskConnectWiFi.enableIfNot();
    taskConnectWiFi.forceNextIteration();
  }
...
bool readConfigFS() {
  ...
  if (SPIFFS.exists("/config.json")) {
    ...
    if (configFile) {
      ...
      std::unique_ptr<char[]> buf(new char[size]);
      configFile.readBytes(buf.get(), size);
      DynamicJsonBuffer jsonBuffer(JSON_OBJECT_SIZE(6)+600);
      //StaticJsonBuffer<JSON_OBJECT_SIZE(6)+600> jsonBuffer;
      JsonObject& json = jsonBuffer.parseObject(buf.get());
      DEBUG_PRINT("Config: ");
      #ifdef SERIALDEBUG
        json.printTo(Serial);
